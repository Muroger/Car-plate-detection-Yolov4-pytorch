...
from utils.detect_plate_number import recognize_plate
# Set printoptions
...
def plot_one_box(x, img, color=None, label=None, line_thickness=None, read_plate=True):
    ...
    if read_plate:
        height_ratio = int(img.shape[1] / 25)
        plate_number = recognize_plate(img, (int(x[0]), int(x[1]), int(x[2]), int(x[3])))
        if plate_number != None:
            cv2.putText(img, plate_number, (int(x[0]), int(x[1]-height_ratio)), 
